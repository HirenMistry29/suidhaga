...
const AddJobCard: React.FC<AddJobCardProps> = ({ isOpen, onClose }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if(imageBase64 !=="" || imageBase64 !==null || imageBase64 !== undefined){const jobInput = {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit} className='overflow-auto custom-scrollbar '>
          ...
          <input type="file" id="imageInput" className="hidden" onChange={handleImageChange} />
                <label htmlFor='imageInput' className="block w-full h-32 px-[5%] text-wrap text-center bg-[#fcfcfc] border-gray-400  border-2 border-dashed rounded-lg cursor-pointer flex items-center justify-center">
      ...
      <div className=''>
        ...
        <div className='mb-4'>
          <p className='text-gray-600 mb-2'>{formik.values.jobDescription}</p>
          <div className='flex border-t border-gray-600 py-2'>
            <span className='text-black'>Color: </span>
            <span className='ml-auto text-black'>{formik.values.color}</span>
          </div>
          <div className='flex border-t border-gray-600 py-2'>
            <span className='text-black'>Size: </span>
            <span className='ml-auto text-black'>{formik.values.size}</span>
          </div>
          <div className='flex border-t border-b mb-6 border-gray-600 py-2'>
            <span className='text-black'>Quantity: </span>
            <span className='ml-auto text-black'>{formik.values.quantity}</span>
          </div>
        </div>
        <div className='flex flex-col md:flex-row items-center'>
          ...
          <div className='flex items-center md:ml-auto'>
            ...
            <button className='ml-2 w-full bg-[#C84869] border-2 py-2 px-6 focus:outline-none hover:bg-[#A72447] rounded text-white font-semibold'
             type='submit' onClick={(event: React.MouseEvent<HTMLButtonElement>) => {formik.handleSubmit()}}>
              Create
            </button>
          </div>
        </div>
